...
class ChronosTrainer:
    ...
    def _collect_router_probs(self) -> torch.Tensor:
        ...
        for layer in self.model.model.layers:
            if isinstance(layer.mlp, ChronosMOEFeedForward):
                p = layer.mlp.last_router_probs  # [B, S, E]
                if p is not None:
                    probs.append(p)
        if not probs:
            return None
        return torch.stack(probs, dim=2)  # [B, S, L, E]
    ...
    def train_step(self, input_ids, labels, step, total_steps):
        ...
        with self.autocast_ctx:
            ...
            # Temporal locality loss: use per-layer router probs averaged over layers
            router_probs_4d = self._collect_router_probs()  # [B, S, L, E]
            if router_probs_4d is not None and router_probs_4d.shape[1] > 1:
                # Average over layers → [B, S, E], then compute temporal penalty
                router_probs_mean = router_probs_4d.mean(dim=2)
                loss = total_loss(
                    ce_loss, aux_loss,
                    router_probs_mean,
                ...
                loss = ce_loss + self.config.lambda_balance * aux_loss
        ...
        return loss.item() * self.args.accumulation_steps, ce_loss.item(), aux_loss.item()
    ...
    def train_epoch(self, epoch, loader, iters, start_step=0, wandb=None):
        ...
        for step, (input_ids, labels) in enumerate(loader, start=start_step + 1):
            loss, ce_loss, aux_loss = self.train_step(
