...
class ChronosInferenceEngine:
    ...
    def generate(
        ...
        for step in range(max_new_tokens):
            # Get availability masks: prefer PrefillScheduler over CacheManager
            if scheduler is not None:
            ...
            # Advance prefetch (CacheManager path only — scheduler already loaded)
            if scheduler is None:
                self.cache_manager.step(lookahead_probs, current_ids)
